...
class TransformerDecoderLayerGlobalImproved(Module):
    ...
    def forward(self, tgt, memory, memory2=None, tgt_mask=None, tgt_key_padding_mask=None, *args, **kwargs):
        '''tgt(src): all zero memory(z): from data'''
        #print('memory.shape: ',memory.shape)
        '''memory.shape:  torch.Size([1, 10, 256])'''
        #print('tgt: ',tgt)
        #print('tgt.shape: ',tgt.shape)
        '''tgt.shape:  torch.Size([60, 10, 256])'''
        tgt1 = self.norm1(tgt)
        #print('tgt1.shape: ',tgt1.shape)
        '''tgt1.shape:  torch.Size([60, 10, 256])'''
        tgt2 = self.self_attn(tgt1, tgt1, tgt1, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask)[0]
        #print('tgt2.shape: ',tgt2.shape)
        '''tgt2.shape:  torch.Size([60, 10, 256])'''
        tgt = tgt + self.dropout1(tgt2)
        #print('tgttgt.shape: ',tgt.shape)
        '''tgttgt.shape:  torch.Size([60, 10, 256])'''
